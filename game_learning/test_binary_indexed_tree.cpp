...
public:
	BinaryIndexedTree(vector<int>& nums) {
		...
		*/
		...
		n = nums.size();
		tree.resize(n+1, 0);
		copy(nums.begin(), nums.end(), tree.begin() + 1);
		for (int i = 1; i <= n; ++i) {
			int parent = i + lsb(i);
	...
	void update(int idx, int val) {
		int oldVal = queryRange(idx, idx);
		// tree is index 1 based, nums is index 0 based
		++idx;
...
int main()
{
	...
	tree.printTree();
	cout << "queryRange(1, 4): " << tree.queryRange(1, 4) << endl;
	cout << "queryRange(5, 5): " << tree.queryRange(5, 5) << endl;
	tree.update(2, 6);
	tree.printTree();
	return 0;
}
